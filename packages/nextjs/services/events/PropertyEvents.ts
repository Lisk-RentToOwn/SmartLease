...
export class PropertyEventService {
  ...
  constructor(
    ...
    this.contracts = {
      ...
      PropertyToken: new Contract(
        propertyTokenAddress,
        propertyTokenAbi,
        PROVIDER
      ),
      IdentityProvider: new Contract(
  ...
  private initializeWebSocket() {
    if (typeof window === "undefined") {
      throw new Error(
        "WebSocketProvider cannot be used in server-side environment"
      );
    }
    ...
    if (!this.wsContracts && this.wsProvider) {
      this.wsContracts = {
        RentToOwn: new Contract(
          this.rentToOwnAddress,
          this.rentToOwnAbi,
          this.wsProvider
        ),
        PropertyToken: new Contract(
        ...
        IdentityProvider: new Contract(
          this.identityProviderAddress,
          this.identityProviderAbi,
          this.wsProvider
        ),
      };
    }
  ...
  async getEvents(
    ...
    const totalEvents = await this.contracts[contractName].queryFilter(
      filter,
      fromBlock,
      toBlock
    );
  ...
  async getTenantAssignments(
    landlordAddress: string
  ): Promise<
    { propertyId: number; tenants: string[]; propertyName: string }[]
  > {
    ...
    return createdEvents.map((propEvent) => {
      const propertyId = Number(
        propEvent.args.propertyId?.toString?.() ?? propEvent.args[0]
      );
      const propertyName =
        propEvent.args.name?.toString?.() ?? propEvent.args[5];
      ...
      const tenants = occupiedEvents
        ...
        .map(
          (e) => e.args.tenant?.toLowerCase?.() ?? e.args[1]?.toLowerCase?.()
        );
  ...
  async getRentPaymentHistory(
    ...
    if (filters.landlord) {
      const landlordProperties = await this.getLandlordProperties(
        filters.landlord
      );
  ...
  > {
    ...
    allRentEvents.forEach((event) => {
      ...
      if (date.getFullYear() === currentYear) {
        ...
        monthlyData[month].collected += Number(
    ...
    for (const property of properties) {
      const tenantsForProperty = tenantAssignments.find(
        (a) => a.propertyId.toString() === property.args.propertyId.toString()
      );
  ...
  async getTokenDistribution(
    tokenId: number
  ): Promise<{ holder: string; percentage: number }[]> {
  ...
  async getPropertyTimeline(
    ...
    const eventQueries = [
      this.getEvents(
        ...
        res.events.map((e) => ({ ...e, type: "PropertyCreated" }))
      ),
      this.getEvents(
        "RentToOwn",
        "RentPaid",
      ...
      this.getEvents(
        ...
        res.events.map((e) => ({ ...e, type: "PropertyOccupied" }))
      ),
      this.getEvents(
        "PropertyToken",
        "PropertyTokenMinted",
  ...
  async getEquityDistribution(
  ...
  ): Promise<{ month: string; equity: number }[]> {
    // Specify "RentToOwn" as the contract name for these events
    const { events } = await this.getEvents("RentToOwn", "EquityUpdated", {
      propertyId,
    });
    ...
    const monthlyData = Array.from({ length: 12 }, (_, i) => {
      ...
      const totalEquity = monthEvents.reduce(
        (sum, e) => sum + Number(e.args.newEquity),
        0
      );
  ...
  async getTenantEquityUpdates(
    ...
    const filters: { tenant: string; propertyId?: number } = {
  ...
  calculateVestingSchedule(
    ...
    return {
      ...
      fullOwnershipDate:
        paymentCount >= duration
          ? new Date(
              lastPayment.setMonth(
                lastPayment.getMonth() + (duration - paymentCount)
              )
            )
          : null,
    };
  ...
  async getTenantPaymentHistory(
    ...
    const filters = [propertyId, tenantAddress];
    if (propertyId !== undefined) filters[0] = propertyId;
