...
export class PropertyEventService {
  ...
  async getLandlordProperties(
    ...
    const { events } = await this.getEvents("RentToOwn", "PropertyCreated", {
  ...
  async getTenantAssignments(
    landlordAddress: string
  ): Promise<{ propertyId: number; tenants: string[] }[]> {
  ...
  > {
    ...
    properties.forEach((property) => {
      ...
      const monthlyRent =
        Number(ethers.formatUnits(property.args.value, 18)) / 12;
  ...
  async getTenantPaymentHistory(
    ...
    const filters = [propertyId ?? null, tenantAddress];
